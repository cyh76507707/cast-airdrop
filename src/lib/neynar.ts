const NEYNAR_API_KEY =
  process.env.NEYNAR_API_KEY || "7302B58C-FDC7-44B9-93F0-B6225E2AE889";
...
export async function sendNeynarMiniAppNotification(params: {
  ...
  console.log(
    `Sending notification to FID ${params.fid}: ${params.title} - ${params.body}`
  );
...
export async function getCastByHashOrUrl(
  identifier: string,
  type: "hash" | "url" = "hash"
) {
...
export async function getCastReactions(hash: string) {
  ...
  const response = await fetch(
    `https://api.neynar.com/v2/farcaster/reactions/cast/?hash=${encodeURIComponent(
      hash
    )}&types=likes,recasts&limit=100`,
    {
      headers: {
        "x-api-key": NEYNAR_API_KEY,
      },
...
export async function getCastComments(hash: string, authorFid?: number) {
  console.log(
    "Fetching cast comments (replies) for hash:",
    hash,
    "fid:",
    authorFid
  );
  ...
  const response = await fetch(
    `https://api.neynar.com/v2/farcaster/cast/conversation/?identifier=${encodeURIComponent(
      hash
    )}&type=hash&reply_depth=1&include_chronological_parent_casts=false&limit=50`,
    {
      headers: {
        "x-api-key": NEYNAR_API_KEY,
      },
...
export async function getCastQuotes(hash: string) {
  ...
  const response = await fetch(
    `https://api.neynar.com/v2/farcaster/cast/quotes?identifier=${encodeURIComponent(
      hash
    )}&type=hash&limit=50`,
    {
      headers: {
...
export async function getAllCastUsers(
  urlOrHash: string
    ...
    if (cast.replies?.count > 0) {
      try {
        ...
        console.log("Comments data structure:", {
          ...
          conversationKeys: commentsData.conversation
            ? Object.keys(commentsData.conversation)
            : [],
          resultKeys: commentsData.result
            ? Object.keys(commentsData.result)
            : [],
          castsLength: commentsData.casts ? commentsData.casts.length : 0,
          castsType: commentsData.casts
            ? typeof commentsData.casts
            : "undefined",
        });

        // v1 API 응답 구조: { result: { casts: [...] } }
        if (
          commentsData.result?.casts &&
          Array.isArray(commentsData.result.casts)
        ) {
          ...
          commentsData.casts.forEach((reply: any, index: number) => {
            console.log(`Processing comment ${index}:`, {
              ...
              hasVerifiedAddresses:
                !!reply.author?.verified_addresses?.primary?.eth_address,
            });
            const user = transformToCastUser(reply.author);
            if (user) {
              comments.push(user);
              console.log(
                `Added comment user: ${user.username} (FID: ${user.fid})`
              );
            } else {
              console.log(
                `Skipped comment user: ${reply.author?.username} (FID: ${reply.author?.fid})`
              );
            }
          });
        }
        // v2 conversation 구조 - conversation.cast.replies
          ...
          commentsData.conversation?.cast?.replies &&
          Array.isArray(commentsData.conversation.cast.replies)
        ) {
          console.log(
            "Found comments (v2 cast.replies):",
            commentsData.conversation.cast.replies.length
          );
        ...
        // v2 conversation 구조 - conversation.replies
          ...
          commentsData.conversation?.replies &&
          Array.isArray(commentsData.conversation.replies)
        ) {
          console.log(
            "Found comments (v2 replies):",
            commentsData.conversation.replies.length
          );
        ...
        // v2 conversation 구조 - conversation.direct_replies
          ...
          commentsData.conversation?.direct_replies &&
          Array.isArray(commentsData.conversation.direct_replies)
        ) {
          console.log(
            "Found comments (v2 direct_replies):",
            commentsData.conversation.direct_replies.length
          );
        ...
        // v2 conversation 구조 - conversation.cast.direct_replies
          ...
          commentsData.conversation?.cast?.direct_replies &&
          Array.isArray(commentsData.conversation.cast.direct_replies)
        ) {
          console.log(
            "Found comments (v2 cast.direct_replies):",
            commentsData.conversation.cast.direct_replies.length
          );
          commentsData.conversation.cast.direct_replies.forEach(
        ...
        // 마지막 시도: conversation 객체 자체가 배열인 경우
          ...
          console.log(
            "Found comments (conversation array):",
            commentsData.conversation.length
          );
