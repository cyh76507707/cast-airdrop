...
export function WalletConnect({ onWalletConnect, onWalletDisconnect, currentWallet }: WalletConnectProps) {
  ...
  useEffect(() => {
    ...
    const checkFrame = async () => {
      try {
        ...
        if (typeof window !== 'undefined' && window.location.href.includes('farcaster.xyz')) {
          setIsInFrame(true);
        } else {
    ...
    return (
      <Card className="max-w-md mx-auto mb-6">
        <CardHeader>
          ...
          <CardDescription>
            {currentWallet.type === 'farcaster' ? 'Farcaster wallet' : 'MetaMask'} is connected.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                <strong>Address:</strong> {currentWallet.address.substring(0, 6)}...{currentWallet.address.substring(-4)}
              </p>
              <p className="text-sm text-green-700">
                <strong>Type:</strong> {currentWallet.type === 'farcaster' ? 'Farcaster' : 'MetaMask'}
              </p>
            </div>
            <Button 
    ...
    <Card className="max-w-md mx-auto mb-6">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3">
          ...
          >
            {isInFrame ? '🔗 Connect Farcaster Wallet' : '❌ Only available in Farcaster app'}
          </Button>
          ...
          >
            {isMetamaskAvailable ? '🦊 Connect MetaMask Wallet' : '❌ MetaMask installation required'}
          </Button>
        </div>
